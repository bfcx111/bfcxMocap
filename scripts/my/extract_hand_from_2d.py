...
def detectHandsKpts(results, hand_net, frame, args):
    ...
    if args.merge and bbox_iou(results['handl'], results['handr'])>args.iou:
...
def main(root, hand_ckpt, args):
    ...
    def get_ann_img(root, cam, annid):
        ...
        if not os.path.exists(imgname):
            print('img error:',imgname)
            return False, None, None
        return True, data, cv2.imread(imgname)
        
    camlist = sorted(os.listdir(join(root, 'annots')))
    os.makedirs(join(root, 'annots2'), exist_ok=True)
    exist = sorted(os.listdir(join(root, 'annots2')))
...
if __name__ == '__main__':
    ...
    hand_ckpt = '/dellnas/users/shuaiqing/cache-easypose-6/hrnet+hand+320x320/model/last.ckpt'
    with Timer('body2hand'):
        main(root=args.path, hand_ckpt=hand_ckpt, args=args)
        # main(root=args.path, hand_ckpt=hand_ckpt, args=args)
